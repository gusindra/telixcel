<div>
    ...
    <!-- Form Action Modal -->
    <x-jet-dialog-modal wire:model="modalActionVisible" maxWidth="3xl">
        <x-slot name="title">
            {{ __('New User') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4 p-3 grid grid-cols-3 gap-3 items-end">
                <div class="col-span-2">
                    <x-jet-label for="input.password" value="{{ __('Password') }}" />
                    <div class="relative" x-data="{ input: 'password' }">
                        <input autocomplete="off" id="input.password" class="p-2 pr-16 border dark:bg-slate-800 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full" wire:model.debunce.800ms="input.password" autofocus name="input.password" type="password" x-bind:type="input">
                        <button type="button" class="absolute inset-y-0 right-0 mt-1 px-3 text-xs text-gray-500 uppercase" x-on:click="input = (input === 'password') ? 'text' : 'password'" x-text="input == 'password' ? 'show' : 'hide'">show</button>
                    </div>
                    <x-jet-input-error for="input.password" class="mt-2" />
                </div>
                <div class="col-span-1">
                    <x-jet-secondary-button wire:click="generatePassword" wire:loading.attr="disabled">
                        {{ __('Auto Password') }}
