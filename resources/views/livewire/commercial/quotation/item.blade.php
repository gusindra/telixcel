                                            ...
                                            <tr>
                                                ...
                                                <td class="items-center justify-end py-3 text-right {{$data->status=='draft'?'':'hidden'}}">
                                                    <div class="items-center flex">
                                                        <a href="#" title="{{ __('Edit') }}" class="cursor-pointer m-2 w-auto text-xs p-1 bg-yellow-200 text-gray-800 rounded" wire:click.prevent="updateShowModal('{{ $item->id }}')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                                                            <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                                            <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                                            </svg>
                                                        </a>
                                                        <a href="#" title="{{ __('Delete') }}" class="cursor-pointer m-2 w-auto text-xs p-1 bg-red-200 text-gray-800 rounded" wire:click.prevent="deleteShowModal('{{ $item->id }}')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4 p-3 grid grid-cols-3">
                <div class=" ">
                    ...
                    <x-jet-input id="price" type="text" class="mt-1 block w-full" wire:model.defer="price" autofocus />
                    <x-jet-input-error for="price" class="mt-2" />
                </div>
                <div class="mx-3">
                    <x-jet-label for="qty" value="{{ __('Qty') }}" />
                    <x-jet-input id="qty" type="text" class="mt-1 block w-full" wire:model.defer="qty" autofocus />
                    <x-jet-input-error for="qty" class="mt-2" />
                </div>
                <div class="mx-3">
                    <x-jet-label for="unit" value="{{ __('Unit Measurement') }}" />
                    <x-jet-input id="unit" placeholder="item / meter / kg / etc" type="text" class="mt-1 block w-full" wire:model.defer="unit" autofocus />
                    <x-jet-input-error for="unit" class="mt-2" />
                </div>
            </div>
            <div class="col-span-6 sm:col-span-4 p-3">
                <x-jet-label for="description" value="{{ __('Description') }}" />
                <x-jet-input id="description" type="text" class="mt-1 block w-full" wire:model.defer="description" autofocus />
