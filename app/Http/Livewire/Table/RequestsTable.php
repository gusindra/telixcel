...
class RequestsTable extends LivewireDatatable
{
    ...
    public function builder()
    {
        if(auth()->user()->super && auth()->user()->super->first() && auth()->user()->super->first()->role == 'superadmin'){
            return Request::query()->with('agent', 'client')->orderBy('created', 'desc');
        }
        return Request::query()->where('requests.user_id', auth()->user()->currentTeam->user_id)->with('agent', 'client')->orderBy('created', 'desc');
    }
